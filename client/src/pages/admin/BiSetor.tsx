...
export default function AdminBiSetor() {
  const utils = trpc.useUtils();
  const { data, isLoading } = trpc.biSetor.listAll.useQuery();
  const createMutation = trpc.biSetor.create.useMutation({
    onSuccess: () => utils.biSetor.invalidate(),
  });
  const updateMutation = trpc.biSetor.update.useMutation({
    onSuccess: () => utils.biSetor.invalidate(),
  });
  const deleteMutation = trpc.biSetor.delete.useMutation({
    onSuccess: () => utils.biSetor.invalidate(),
  });
